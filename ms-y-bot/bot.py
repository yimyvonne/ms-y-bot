...
class Bot:
    ...
    def get_accounts(self, user, target):
        ...
        logger.info("getting {} from user: {}".format(target, user))
        # waiter.find_element(self.driver, "//div[@role='dialog']", XPATH)
        search_xpath = "//a[contains(@href,'/following')]" if target == "following" else  "//a[contains(@href,'/followers')]"
        WebDriverWait(self.driver, 20).until(
            expected_conditions.element_to_be_clickable((By.XPATH, search_xpath))).click()
        wait(3, 5, "clicked on {} pop up".format(target))
    ...
    def check_for_warning(self):
        if self.find_user_profile_element(By.XPATH,
            ...
            wait(3600, 3600, "warning detected, holding application for an hour for intervention")
            self.driver.quit()
            return True
    ...
    def follow_followers(self):
        ...
        for username in follower_list:
            ...
            try:
                ...
                with open("[Followed][{}]".format(self.username), "r+") as flist:
                    ...
                    if username not in lines:
                        ...
                        if self.find_user_profile_element(By.XPATH, "(//*[contains(text(), 'Try again later')])"):
                            logger.fatal("!!!!! Temporarily blocked. Retry in a few hours. !!!!!!")
                            wait(3600, 3600, "warning detected, holding application for an hour for intervention")
                            self.driver.quit()
                            break
                ...
                wait(30, 120, "requested to follow {}".format(username))
    ...
    def like_first_six_pics(self, user, failed_to_scout):
        ...
        for i in range(0, 6 if len(images) >= 6 else len(images)):
            ...
            #better XPATH
            try:
                like_button = WebDriverWait(self.driver, 5).until(expected_conditions.element_to_be_clickable((By.XPATH, "/html/body/div[6]/div[3]/div/article/div/div[2]/div/div/div[2]/section[1]/span[1]/button")))
    ...
    def get_number_on_profile(self, target):
        try:
            number_string = WebDriverWait(self.driver, 3).until(expected_conditions.element_to_be_clickable(
                (By.XPATH, "//a[contains(@href,'/{}')]".format(target)))).get_attribute("text").split(" ")[0]
            # avoid unfollowing other business accounts
            multipler = 1000 if 'k' in number_string else 10000000
            return float(number_string[:len(number_string) - 1]) * multipler \
                if 'k' in number_string or 'm' in number_string else int(number_string.replace(",", ""))
        except TimeoutException:
            return 0


    def unfollow_unsuccessfully_scouted(self):
        current_following = self.get_accounts(self.username, "following")
        current_followers = self.get_accounts(self.username, "followers")
        following_on_profile = self.get_number_on_profile("following")
        followers_on_profile = self.get_number_on_profile("followers")
        if (following_on_profile-len(current_following) < abs(15)) and (followers_on_profile-len(current_followers) < abs(15)):
            for user in current_following:
                if user not in current_followers:
                    self.go_to_user(user)
                    try:
                        unfollow_button = self.driver.find_element(By.CSS_SELECTOR, "[aria-label='Following']")
                        follower_number = self.get_number_on_profile("followers")
                        if follower_number < 2000:
                            unfollow_button.click()
                            self.driver.find_element(By.XPATH, "(//button[contains(text(),'Unfollow')])").click()
                            time.sleep(1)
                            if self.find_user_profile_element(By.XPATH, "(//*[contains(text(), 'Try again later')])"):
                                logger.fatal("!!!!! Temporarily blocked. retry in a few hours. !!!!!!")
                                wait(3600, 3600, "warning detected, holding application for an hour for intervention")
                                self.driver.quit()
                                break
                            wait(10, 30, "unfollowed {}".format(user))
                    except NoSuchElementException:
                        pass
        else:
            logger.error("cannot perform unfollow. Reported following: {} but only returned {}; "
                      "Reported followers: {} but only returned {}".format(following_on_profile, len(current_following), followers_on_profile, len(current_followers)))
            target = input("Unfollowing completed. Scrap and follow initiated. Please input target username: ")
        target = input("Unfollowing completed. Scrap and follow initiated. Please input target username: ")
        scrap_and_follow(self, target)
...
def scrap_and_follow(bot, target):
    bot.get_accounts(target, "followers")
